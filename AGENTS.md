...
**Portraits** are written by an LLM that reads sample traces from each region and produces: a name, a multi-paragraph description, a signature sentence, defining verbs, and borders. The portrait is interpretive — it is the atlas's attempt to articulate what the region IS.

**Calibration** establishes the atlas's sense of self. Novelty thresholds tell you whether new text is familiar (activates known territory), edge (at the boundary of known space), or alien (outside the atlas entirely). Border detection tells you when text sits between two regions — cognitive liminal space.

**Centroids** are the geometric center of each region in embedding space. They are how the atlas recognizes what belongs where. Stored as base64 float32 in atlas.json.

## What Is Invariant (Do Not Break)

- **atlas.json format**: version, metadata, regions[], calibration, centroids_b64. This is the protocol. Other tools, agents, and atlases depend on this structure.
...
- **Region structure**: id, name, portrait, signature, verbs, borders, size, fraction, sources. Consumers of atlas.json parse these fields.
...
This defines what the atlas **sees** in each region. The current prompt asks for tensions, patterns, obsessions, code-switching. You could:
...
Changing the portrait prompt changes the atlas's interpretive layer without changing the geometric structure.

### Portrait Model (`ATLAS_PORTRAIT_MODEL`)
...
### Input Sources
atlas.py currently handles Twitter archives and generic JSONL. Any text can be a trace. To add new sources:
...
`classify_text()` takes any text and returns: top 3 matching regions, border detection, novelty score. This is how an agent can **route through the atlas**. Ideas:
- Use the atlas as a routing layer for agent behavior — respond differently depending on which region the user's message activates
...
- **Shadow detection**: Find what the atlas DOESN'T contain. Cluster the noise, the low-activation regions, the verbs that never appear. The absence is as diagnostic as the presence.
- **Agent identity layer**: An agent reads its user's atlas.json and shapes its behavior accordingly — not through a system prompt written by hand, but through empirical cognitive topology derived from what the human actually did.

## File Structure

One file: `atlas.py`. All logic is here. Sections:
...
5. **Output** (~lines 519-632): atlas.json, atlas.md, atlas.html (Plotly)
