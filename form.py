#!/usr/bin/env python3
"""
form.py v0.1 — Cognitive Atlas Builder

Build a cognitive atlas from your behavioral traces. $0. Under 30 minutes.

    python form.py --traces ~/Downloads/twitter-archive/   # Twitter export
    python form.py --traces my_data.jsonl                   # generic JSONL
...
VERSION = "0.1.0"
EMBED_MODEL = os.environ.get("FORM_EMBED_MODEL", "gemini-embedding-001")
PORTRAIT_MODEL = os.environ.get("FORM_PORTRAIT_MODEL", "gemini/gemini-3-flash-preview")
...
EMBED_TRUNCATE = 2000
ANNOTATE_MODEL = os.environ.get("FORM_ANNOTATE_MODEL", PORTRAIT_MODEL)
ANNOTATE_MAX_WORKERS = int(os.environ.get("FORM_ANNOTATE_WORKERS", "10"))
...
def save_form_json(portraits, centroids_normed, calibration, metadata, output_dir):
    ...
    path = Path(output_dir) / "form.json"
    with open(path, "w") as f:
        json.dump(atlas, f, indent=2, ensure_ascii=False)
    size_kb = path.stat().st_size / 1024
    print(f"  form.json ({size_kb:.0f} KB)")
    return path


def save_form_md(portraits, n_traces, output_dir):
    parts = ["# COGNITIVE ATLAS\n"]
    parts.append(f"*{n_traces:,} traces across {len(portraits)} regions. "
                 f"Generated by form.py v{VERSION}.*\n")
    parts.append("---\n")
...
def save_form_html(traces, coords_2d, labels, portraits, n_clusters, output_dir):
    ...
    path = Path(output_dir) / "form.html"
    with open(path, "w") as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="form.py v0.1 — Cognitive Atlas Builder",
    ...
    parser.add_argument("--classify", help="Classify text against a cognitive atlas")
    parser.add_argument("--form", default="form.json", help="Path to form.json")
    ...
    parser.add_argument("--version", action="version", version=f"form.py v{VERSION}")
    ...
    if args.classify:
        atlas = load_form_json(args.form)
        client = genai.Client()
    ...
    print(f"form.py v{VERSION} — Cognitive Atlas Builder ({mode_label})")
    print("=" * 55)
    t_start = time.time()
    ...
    save_form_json(portraits, centroids_normed, calibration, metadata, out)
    save_form_md(portraits, len(traces), out)

    if not args.no_viz:
        save_form_html(traces, coords_2d, labels, portraits, n_clusters, out)

    elapsed = time.time() - t_start
    print(f"\nDone in {elapsed/60:.1f} minutes.")
    print(f"  To classify: python form.py --classify \"your text\" --form {out}/form.json")
